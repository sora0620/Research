...
class BC:
    ...
    # 媒介中心性の高い順にノードの周辺エッジ (入エッジ, 出エッジ両方) を取得していく
    def betweenness_centrality_in_out_sampling(self, origin_graph, graph_name, sampling_rate):
        ...
        path = "../../../Dataset/BC/{}.json".format(graph_name)
        bc_dict = {}
        
        with open(path, 'r') as f:
            bc_dict = json.load(f)
        
        bc_sorted = sorted(bc_dict.items(), key=lambda x:x[1], reverse=True)
        bc_sorted_node_list = [node for node, bc in bc_sorted]
        sampling_edge_set = set()
        
        while len(sampling_edge_set) < sampling_edge_num:
            current_node = int(bc_sorted_node_list.pop(0)) # 現存ノードの中で最も BC が高いノードを取得 & リストから削除
        ...
        print("BC_in_out_Time  :", tim)
              
        return self.sampling_graph
    
    # 入次数を取得
    def betweenness_centrality_in_sampling(self, origin_graph, graph_name, sampling_rate):
        time_start = time.perf_counter()
        ...
        time_end = time.perf_counter()
        tim = time_end - time_start
        print("BC_in_Time  :", tim)
              
        return self.sampling_graph
    
    # 出次数を取得
    def betweenness_centrality_out_sampling(self, origin_graph, graph_name, sampling_rate):
        time_start = time.perf_counter()
