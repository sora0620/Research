    ...
    if(!ifs){
        ...
        while (getline(ifs, line)){
            ...
            for (int i = 0; i < node_vec.size(); i++) {
                ...
                this->node_list_set.insert(node_vec[i]);

                if (i != 0) {
                    this->adjacency_list[node_vec[0]].push_back(node_vec[i]);
                    this->in_neighbor_list[node_vec[i]].push_back(node_vec[0]);
                }
            }
        }
...
unordered_map<int, double> Graph::pagerank()
{
    ...
    for (int i = 0; i < MAX_ITER; i++) {
        ...
        for (auto [prev_node, adj_vec] : out_neighbor_map) {
            ...
            if (out_deg == 0) {
                dangling_score += ALPHA * prev_score[prev_node] / node_num;
            } else {
                for (int out_node : adj_vec) {
                    score[out_node] += ALPHA * prev_score[prev_node] / out_deg;
                }
            }
        }
