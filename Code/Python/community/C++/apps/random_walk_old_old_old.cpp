...
unordered_map<int, int> visited_count_from_one_node(unordered_map<int, unordered_set <int> > adj_map, int start_node, int random_walk_num)
{
...
unordered_map<int, unordered_map <int, int> > visited_count_from_all_node(Graph graph, unordered_set<int> change_node_set, int change_flow)
{
    ...
    unordered_map<int, unordered_set <int> > adj_map = graph.get_adjacency_list();
    ...
    // count_map 競合しない？ 元々要素なしの map だと作成のために map の破壊が起きるから駄目っぽい
    for (int i = 0; i < parallel_vec.size(); i++) {
        int node = parallel_vec.at(i);
        count_map[node] = visited_count_from_one_node(adj_map, node, flow_map[node]);
        cout << node << endl;
    }
