...
// const 参照の隣接リストを返す
const unordered_map<int, unordered_set <int> > Graph::get_adjacency_list()
{
...
unordered_map<int, double> Graph::pagerank()
{
    ...
    unordered_map<int, unordered_set <int> > out_neighbor_map = get_adjacency_list(); // キーにノード, 値に出隣接ノードのリストの辞書
