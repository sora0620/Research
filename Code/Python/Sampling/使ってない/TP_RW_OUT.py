        ...
        while len(list(self.sampled_graph)) != nodes_to_sample: # ノード数が規定サンプル数に達したら終了
            ...
            while True:
                ...
                for i in range(seed_num):
                    ...
                    edge_list = list(complete_graph.successors(current_node))
                    if len(edge_list) != 0:
                        chosen_node = random.choice(edge_list)
                        if len(list(self.sampled_graph)) != nodes_to_sample:
                            self.sampled_graph.add_edge(current_node, chosen_node)
