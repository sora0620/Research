...
def return_sampling_graph(graph, connect_node_list, sampling_rate, sampling_method, weight_dict, default_weight, ppr_dict):
    sampling_graph = nx.DiGraph()
    if sampling_method == "FC":
        obj = sp.FC()
        sampling_graph = obj.flow_control_sampling_2(graph, sampling_rate, connect_node_list, weight_dict, default_weight, ppr_dict)
...
def main_ken():
    ...
    for sampling in sampling_list:
        ...
        for rate in rate_list:
            sampling_graph = return_sampling_graph(origin_graph, connect_node_list, rate, sampling, weight_dict, default_weight, ppr_dict)
...
def main_rmse():
    ...
    for sampling in sampling_list:
        ...
        for rate in rate_list:
            sampling_graph = return_sampling_graph(origin_graph, connect_node_list, rate, sampling, weight_dict, default_weight, ppr_dict)
            ppr_sum_origin, ppr_sum_sampling = return_ppr_sum_rank(origin_graph, sampling_graph, connect_node_list, weight_dict)
            rmse_list.append(Calc.calc_rmse_top(ppr_sum_origin, ppr_sum_sampling, top_node_num))
