...
def write_in_txt_graph(G, filename): # グラフを .txt に書き込む. 隣接行列形式にする (孤立ノードの情報も書き込めるので).
    f = open(filename, 'w')
...
def dcg(approx_nodes, exact_dict, k): # べき乗型の DCG の計算
...
def dcg_perfect(exact_dict, k): # DCG を正規化するために利用する理想的なランキング指標
...
def calc_ndcg(approx_nodes, exact_dict, k): # DCG を正規化して NDCG を計算
    return dcg(approx_nodes, exact_dict, k) / dcg_perfect(exact_dict, k)
...
def calc_synthesis_pr_ndcg(sampled_graph, pr_origin, k_div):
        ...
        return calc_ndcg(approximate, exact, k_size)
...
def read_origin_graph(graph):
    file_name = "../../Dataset/Origin/Graph/{}.adjlist"
...
def read_sampling_graph(div, partition, graph, num, size, sampling, ver):
